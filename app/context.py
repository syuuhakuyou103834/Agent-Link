...
def validate_context(context):
    ...
    for record in records:
        ...
        code = record.get('mode') == 'code'
        if code and len(turns) > 7:
            raise ValueError('项目审查历史超出轮次上限。')
        for i, turn in enumerate(turns):
            role = ('B' if i % 2 else 'A') if code else (initiator if i % 2 == 0 else ('B' if initiator == 'A' else 'A'))
            ...
            if code and turn.get('phase') != ('summary' if i == len(turns)-1 else 'review' if i % 2 else 'implement'):
                raise ValueError('项目审查历史缺少完整阶段或最终总结。')
...
def prepare_context(box, parent_job_id):
    ...
    turns = []
    code = meta.get('mode') == 'code'
    count = 1 + 2 * rounds
    if code:
        count = state.get('calls')
        if (rounds > 3 or type(count) is not int or count not in range(3, 2*rounds+2, 2)
                or meta.get('budget') != 2*rounds+1):
            raise ValueError('项目审查的实际完成步骤数无效。')
    for i in range(count):
        role = ('B' if i % 2 else 'A') if code else (meta.get('initiator') if i % 2 == 0 else ('B' if meta.get('initiator') == 'A' else 'A'))
        step = f'{i:03d}-{role}'
        ...
        item = {'index': i, 'role': role, 'answer': turn.get('answer')}
        if code:
            if turn.get('job_id') != parent_job_id:
                raise ValueError('项目审查历史步骤归属无效。')
            item['phase'] = turn.get('phase')
        turns.append(item)
...
class PromptBudgetError(ValueError):
    def __init__(self, prompt, stage='本轮'):
        self.details = dict(code='context_budget', stage=stage, chars=len(prompt),
                            bytes=len(json.dumps(prompt, ensure_ascii=False).encode('utf-8')),
                            char_limit=280000, byte_limit=950000, request_sent=False)
        super().__init__(f'{stage}上下文超过传输上限：字符 {self.details["chars"]:,}/280,000；'
                         f'JSON 字节 {self.details["bytes"]:,}/950,000。未截断、未发送模型请求。'
                         '完整记录保留；请查看上下文详情，整理必要正文后明确继续。')


def check_prompt(prompt, stage='本轮'):
    ...
    if len(prompt) > 280000 or len(json.dumps(prompt, ensure_ascii=False).encode('utf-8')) > 950000:
        raise PromptBudgetError(prompt, stage)
