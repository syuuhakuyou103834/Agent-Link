...
from .interruption import recovery_parent
...
class ProjectRecovery:
    ...
    def import_recovery(self, plan):
        ...
        if plan.get('parent_evidence') != recovery_parent(self.box, self.box.get(parent, 'meta.json')):
            raise ValueError('原任务终态或停止指令已变化，拒绝恢复。')
