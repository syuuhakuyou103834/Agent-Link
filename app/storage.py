...
def _json_guard(path):
    ...
    with _retry_io(lambda: io_path(lock_path).open('a+b')) as handle:
        ...
        # Windows permits locking beyond EOF. Initialize only through the owning
        # handle: writing before locking races another creator's byte-range lock.
        handle.seek(0, 2)
        if handle.tell() == 0:
            handle.write(b'0')
            handle.flush()
...
class Mailbox:
    ...
    def validate_meta(self, meta, job_id):
        ...
        unified = meta.get('workflow') == 'unified-conversation-v1'
        if unified and (meta.get('initiator') != 'A' or not isinstance(meta.get('task_brief'), dict)):
            raise ValueError('统一任务只能由 A 发起并绑定已确认说明')
        if type(meta.get("rounds")) is not int or not 1 <= meta["rounds"] <= (99 if unified else 8):
        ...
        if meta.get('mode') == 'code':
            ...
            if (meta['protocol'] != 2 or meta['rounds'] > (99 if unified else 3) or type(meta.get('budget')) is not int
