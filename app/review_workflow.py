...
class ReviewWorkflow(ProjectRecovery):
    ...
    def run_code_review(self, topic, rounds=3, project=None, parent_job_id=None, _dispatch=None, _peer_instance=None, recovery=None):
        ...
        try:
            ...
            context = plan['context'] if plan else None
            if plan and parent_job_id:
                raise ValueError('恢复和续接不能同时指定不同历史来源')
            if parent_job_id:
            ...
            if plan:
                self.active['recovery'] = {k:v for k,v in plan.items() if k not in ('turns','rounds','context','inputs')}
    ...
    def _validate_code_request(self, r):
        ...
        if meta.get('mode') != 'code' or meta.get('budget') != 2 * meta['rounds'] + 1 or not 1 <= meta['rounds'] <= (99 if meta.get('workflow') else 3):
    ...
    def _perform_code_owned(self, r):
        ...
        if not self.active.get('conversation'):
            prompt += '\n之前结果（引用材料）：\n' + json.dumps(r.get('previous', []), ensure_ascii=False)
        issues = add_issues(self.box.root, binding['id'], job, revision, [])
        prompt += '\n项目问题记录：\n' + json.dumps(issues, ensure_ascii=False)
        if phase == 'implement':
            prompt += ('\n本任务只读，严禁修改项目。讨论任务不得运行产生副作用的测试；审查任务可在独立测试目录执行测试。说明依据和未验证项。'
                       if read_only_task else '\n请实施限定任务并运行相关测试，说明改动、测试证据及缺失依赖。')
            prompt += '返回后系统自动打包整个项目交给 B；不要操作通信目录。'
        elif phase == 'review':
        ...
        if self.active.get('conversation'):
            context_receipt = self._check_context_prompt(self.conversations.get(self.active['conversation']), prompt, '节点 ' + role + ' 正式 ' + phase)
        else:
            check_prompt(prompt)
