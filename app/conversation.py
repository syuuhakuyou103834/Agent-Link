...
class UnifiedWorkflow:
    ...
    def unified_submit(self, text, identifier=None, message_id=None):
        ...
        with self.conversations.edit(identifier) as value:
            ...
            if control_word(text) in RESUME_WORDS and value['phase'] in FINISHED:
                raise ValueError('该任务已经结束或由用户停止，不能用“继续”复活；新任务请明确描述要求。')
            if role=='A' and value['phase'] in FINISHED:
                value['phase']='clarifying'
                value.pop('user_stopped', None)
    ...
    def unified_control(self, identifier, action):
        ...
        with self.conversations.edit(identifier) as value:
            if action == 'cancel':
                ...
                value['user_stopped'] = True
                if value.get('job') and value['job'] not in value.setdefault('stopped_jobs', []):
                    value['stopped_jobs'].append(value['job'])
                value['pending'] = []
                if value.get('job'):
    ...
    def _unified_compatible(self):
        ...
        if INTERRUPTION_FEATURE not in peer.get('features', []):
            raise ValueError('技术中断恢复需要双方升级至 0.3.19；请更新另一节点后明确继续')
    ...
    def _consume_unified_input(self, value):
        ...
        for key in value['pending']:
            ...
            if word in RESUME_WORDS and value['phase'] in ('paused', 'interrupted', 'chat_interrupted'):
                if value.get('user_stopped'):
                    raise ValueError('用户已经停止此任务，不能恢复。')
                with self.conversations.edit(value['id']) as current:
    ...
    def _start_unified_job(self, value):
        ...
        if value.get('recover') and (value.get('user_stopped') or value.get('job') in value.get('stopped_jobs', [])):
            raise ValueError('用户已经停止此任务，不能恢复。')
    ...
    def _unified_formal(self, value):
        ...
        if state.get('status') in ('failed','cancelled'):
            if self.settings.role == 'A':
                with self.conversations.edit(value['id']) as current:
                    if current.get('user_stopped') or current['phase'] == 'cancelled':
                        return
                    recoverable = state['status']=='failed' or legacy_peer_failure(self.box, meta)
                    current['phase'] = 'interrupted' if recoverable else 'cancelled'
                    current['error'] = state.get('error','')
                    if state.get('interruption'):current['interruption'] = state['interruption']
            return
        turns = self._unified_turns(meta)
        ...
        try:
            ...
            self._formal_failed(value, meta, error)
        finally:
    ...
    def _formal_failed(self, value, meta, error):
        ...
        committed = self.box.get(meta['id'], 'state.json', {})
        with self.conversations.edit(value['id']) as current:
            if current['phase'] in FINISHED or current.get('user_stopped'):
                return
            if committed.get('status') == 'completed':
                return
            info = committed.get('interruption', {})
            is_stopped = committed.get('status') == 'cancelled' and not legacy_peer_failure(self.box, meta)
            current['phase'] = 'cancelled' if is_stopped else 'interrupted'
            current['error'] = committed.get('error', str(error))
            current['interruption'] = info
            hint = '任务已停止，已有记录保留。' if is_stopped else recovery_hint(info)
            append_message(current, 'system', '执行中断：' + current['error'] + '\n' + hint, 'state')
            ...
            try:
                ...
                if value['phase']=='working' and value.get('job'):
                    try:
                        ...
                        self._formal_failed(value, self.box.get(value['job'], 'meta.json'), error)
                    return
            except PromptBudgetError as error:
                self._block_context(value['id'], error)
                self.report_error(error)
    ...
    def sync_conversation(self, identifier):
        ...
        if value.get('job'):
            ...
            if state.get('interruption'):payload['interruption']=state['interruption']
            if (self.connected and payload['phase']=='interrupted'
