...
from app.storage import read_json, io_path
import unittest
...
class InterventionTests(CodeWorkflowTests):
 ...
 def test_new_steer_targets_only_b_and_deduplicates(self):
  ...
  # Commands bind their job at submission. Wait for B to select this job before
  # sending from B; A's RPC may start before the receiving node is admitted.
  until(lambda:self.nodes['B'].active and self.nodes['A'].client.current
        and self.nodes['A'].client.current.turn_id)
