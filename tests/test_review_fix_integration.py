...
class ReviewFixIntegration(CodeWorkflowTests):
    def test_fix_01_recovery_delivers_history_and_queued_inputs_to_real_dispatch(self):
        ...
        def new_job():
            return next((p for p in entries(self.shared/'jobs','iterdir') if p.name!=history['id']),None)
        ...
        parent=new_job();meta=read_json(parent/'meta.json');state=(fs(parent/'state.json')).read_bytes()
        ...
        originals={p.name:fs(p).read_bytes() for p in entries(parent,'glob','input-*.json')}
        ...
        self.assertEqual((fs(parent/'state.json')).read_bytes(),state)
        for name,data in originals.items():self.assertEqual((fs(parent/name)).read_bytes(),data)
    ...
    def test_fix_02_b_initiated_early_completed_review_continues(self):
        ...
        def child():return next((p for p in entries(self.shared/'jobs','iterdir') if p.name!=parent.name),None)
