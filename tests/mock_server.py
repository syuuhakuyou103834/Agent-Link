...
def work(tid, turn_id, prompt, cancel):
    ...
    if '[LARGE_TOOLS]' in prompt and not prompt.startswith('[AGENTLINK_CONVERSATION]'):
        for i in range(19):
            notify('item/completed', tid, turn_id, item={'type':'commandExecution', 'id':'large-'+str(i),
                'command':'offline fixture', 'status':'completed', 'aggregatedOutput':'RAW-LARGE-TOOL-'+('中'*11980), 'exitCode':0})
    if prompt.startswith('[AGENTLINK_CONVERSATION]'):
        record, _ = json.JSONDecoder().raw_decode(prompt.split('\n',1)[1])
        message, _ = json.JSONDecoder().raw_decode(prompt.split('\n当前消息：\n',1)[1])
        users=[m for m in record['messages'] if m['speaker']=='user']
        initial=users[0]['text'];latest=message['text']
        paths=re.findall(r'[A-Za-z]:[\\/][^\r\n`"<>|，。；、]*',initial)
        directory=paths[0].strip() if paths else ''
        match=re.search(r'(\d+)轮',initial)
        brief=record.get('confirmed') or dict(goal=initial,directory=directory,
            permission='edit' if '[EDIT]' in initial else 'review' if directory else 'discuss',
            allowed_changes='仅用户指定范围',acceptance=['模拟验收条件'],review_focus='独立核对结果',round_limit=int(match[1]) if match else 3)
    ...
    if '[REFUSE]' in prompt:
        text = '无法协助该请求。'
    if '[SLOW]' in prompt and 'AGENTLINK_CONVERSATION' not in prompt:
        cancel.wait(1.5)
    for chunk in [text[i:i+5] for i in range(0, len(text), 5)]:
        if cancel.wait(.35 if '[LONG]' in prompt else .001 if '\n本轮范围：\n' in prompt or 'AGENTLINK_' in prompt else delay):
