...
class StorageTests(unittest.TestCase):
    ...
    def test_simultaneous_first_guard_creation_is_serialized(self):
        ...
        for index in range(12):
            path=self.root/('fresh-'+str(index)+'.json')
            barrier=threading.Barrier(6);errors=[];inside=[]
            def contender():
                try:
                    barrier.wait(3)
                    with storage._json_guard(path):
    ...
    def test_unlocked_initialization_denied_by_existing_range_lock(self):
        ...
        with path.open('a+b') as first,path.open('a+b') as second:
            ...
            # write instead targets EOF and would not reproduce this boundary.
            first.write(b'0')
            second.seek(0)
            msvcrt.locking(second.fileno(),msvcrt.LK_NBLCK,1)
            try:
                with self.assertRaises(PermissionError):first.flush()
            finally:
                second.seek(0);msvcrt.locking(second.fileno(),msvcrt.LK_UNLCK,1)
                first.flush()
