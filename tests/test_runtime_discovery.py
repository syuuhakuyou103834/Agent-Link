...
class RuntimeDiscoveryTests(unittest.TestCase):
 def setUp(self):
  self.temp=FixtureTemporaryDirectory();self.root=Path(self.temp.name)
  ...
  fs(self.new.parent).mkdir(parents=True);fs(self.new).write_bytes(b'fixture')
  self.legacy=self.base/'codex.exe';fs(self.legacy).write_bytes(b'legacy')
 def tearDown(self):
 ...
 def test_explicit_custom_and_existing_version_are_preserved(self):
  ...
  old=self.base/'old'/'codex.exe';fs(old.parent).mkdir();fs(old).write_bytes(b'old')
 ...
 def test_settings_migration_preserves_other_fields_and_disk(self):
  data=self.root/'data';fs(data).mkdir();p=data/'settings.json'
  raw={'codex':str(self.legacy),'role':'B','shared_root':str(self.root/'share'),'model':'mock','sandbox':'read-only'}
  fs(p).write_text(json.dumps(raw),encoding='utf-8');before=fs(p).read_bytes()
  ...
  self.assertEqual(fs(p).read_bytes(),before)
