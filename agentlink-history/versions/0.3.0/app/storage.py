...
class Mailbox:
    ...
    def create(self, topic, rounds, initiator="A"):
    ...
    def validate_meta(self, meta, job_id):
        if not isinstance(meta, dict) or meta.get("protocol") != 1 or meta.get("id") != job_id:
            raise ValueError("不兼容的任务记录")
        if not isinstance(meta.get("topic"), str) or len(meta["topic"]) > 24000:
        ...
        if not isinstance(meta.get("expires"), (int, float)) or meta["expires"] < now():
