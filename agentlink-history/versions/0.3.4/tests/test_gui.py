...
try:
    ...
    b.notice.hide()
    b.snapshot['status'] = 'running'
    b.receive('execution', {'role': 'A', 'job_id': b.snapshot['meta']['id'], 'execution': {
        'phase': 'tool_running', 'elapsed_seconds': 7201, 'event_age_seconds': 80,
        'process_alive': True, 'running_tools': 1, 'event_count': 25}})
    assert '工具执行中' in b.execution_label.text() and '120分01秒' in b.execution_label.text()
    assert '继续等待' in b.execution_label.text()
    assert dialog.result_settings().timeout_seconds == 0
    b.snapshot['status'] = 'completed'
    b.render_execution()
    assert '工具执行中' not in b.execution_label.text(), 'Terminal states must not show stale running events'
    saved = b.snapshot
    partial = {'meta': dict(saved['meta']), 'turns': [], 'status': 'cancelled',
               'live': [{'role': 'B', 'step': '000-B', 'index': 0, 'thread_id': 'partial-thread',
                         'answer': '', 'blocks': [{'text': '已写入部分修复 PARTIAL-RECOVERY'}]}]}
    b.snapshot = partial
    export = root / 'partial-export.txt'
    with patch.object(W.QFileDialog, 'getSaveFileName', return_value=(str(export), '')):
        b.export_report()
    assert 'PARTIAL-RECOVERY' in export.read_text(encoding='utf-8-sig')
    assert '未完成输出' in export.read_text(encoding='utf-8-sig')
    assert 'partial-thread' in export.read_text(encoding='utf-8-sig')
    b.snapshot = saved
