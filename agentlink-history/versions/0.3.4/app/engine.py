...
class NodeService:
    ...
    def _admit_start(self, args):
        ...
        try:
            ...
            self.commands.put(('start', dict(args, _dispatch=dispatch)))
    ...
    def _heartbeat(self, force=False):
        ...
        record = {"role": self.settings.role, "host": socket.gethostname(), "pid": os.getpid(),
                  ...
                  "features": [FEATURE, LONG_TASK_FEATURE], "execution": activity}
    ...
    def _pump(self):
        ...
        try:
            if self.connected:
                ...
                peer["online"] = bool(peer.get("updated") and self._observe_peer(peer) < 12 and peer.get("status") != "offline")
                self.emit("peer", peer)
                self._sync_selected()
            self.share_lost = None
    ...
    def run_initiator(self, topic, rounds, parent_job_id=None, _dispatch=None):
        context = None
        peer = read_json(self.box.root / 'nodes' / (self.peer_role + '.json'), {}) or {}
        if (self._observe_peer(peer) >= 12 or peer.get('status') == 'offline'
                or LONG_TASK_FEATURE not in peer.get('features', [])):
            raise ValueError('无时限任务需要双方连接并支持该功能，请将两台更新到 0.3.4 或兼容版本。')
    ...
    def scan_receiver(self):
        peer = read_json(self.box.root / "nodes" / (self.peer_role + '.json'), {}) or {}
        if self._observe_peer(peer) > 12 or peer.get('status') == 'offline' or not peer.get("job_id"):
            return
        job_id = peer["job_id"]
        ...
        try:
            ...
            while True:
                # Read terminal state before _pump/ensure_open so normal completion
                # does not look like cancellation on the reviewing node.
                state = self.box.get(job_id, 'state.json', {}) or {}
                ...
                try:
                    if kind == "connect" and not self.connected:
                        ...
                        try:
                            ...
                            self.run_initiator(args["topic"], args["rounds"], args.get('parent_job_id'), args.get('_dispatch'))
                        finally:
