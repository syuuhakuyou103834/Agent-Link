...
class OverrideTests(unittest.TestCase):
 ...
 def test_optional_nulls_do_not_become_invalid_toml_strings(self):
  result=self.run_case({'mcp_servers':{'node_repl':{'command':'node','tool_timeout_sec':None,'startup_timeout_sec':60.0,'enabled':True,'env':{'MISSING':None,'EMPTY':'','ZERO':'0'}}},'plugins':{}})
  entry=result['mcp_servers']['node_repl']
  self.assertNotIn('tool_timeout_sec',entry)
  self.assertEqual(entry['startup_timeout_sec'],60.0)
  self.assertEqual(entry['env'],{'EMPTY':'','ZERO':'0'})
