...
from .node_input import NodeInput, FEATURE as INPUT_FEATURE
...
class NodeService(ReviewWorkflow, NodeInput):
    ...
    def _check_peer_compatibility(self, peer=None, expected_instance=None):
        ...
        if (type(updated) not in (int, float) or not math.isfinite(updated)
                or not -1 <= now() - updated < 12 or peer.get('status') in (None, 'offline')
                or peer.get('role') != self.peer_role
                or not isinstance(instance, str) or not re.fullmatch('[a-f0-9]{32}', instance)):
            raise ValueError('对端能力记录已失效、离线或实例信息无效，拒绝使用遗留心跳。')
        if expected_instance is not None and instance != expected_instance:
            raise ValueError('对端已更换实例，原场次不能转交给重启后的执行者。')
    ...
    def _heartbeat(self, force=False):
        ...
        record = {"role": self.settings.role, "host": socket.gethostname(), "pid": os.getpid(),
                  ...
                  "features": [FEATURE, SAFETY_FEATURE, LONG_TASK_FEATURE, PROJECT_FEATURE, INPUT_FEATURE], "execution": activity}
    ...
    def _control(self):
        ...
        try:
            ...
            raise Cancelled('对端实例失效，停止继续执行；已发送请求的结果须核查：' + str(error)) from error
    ...
    def _sync_selected(self):
        ...
        value = {"meta": meta, "turns": turns, "live": live, "status": state.get("status", "incomplete"),
                 ...
                 "node_inputs": [read_json(p) for p in sorted(root.glob("input-*.json"))],
        ...
        if meta.get('mode') == 'code':
            ...
            value['project_issues'] = read_json(issues_path, {}).get('issues', [])
