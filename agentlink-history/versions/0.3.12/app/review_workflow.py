...
class ReviewWorkflow:
    ...
    def run_code_review(self, topic, rounds=3, project=None, parent_job_id=None, _dispatch=None, _peer_instance=None):
    ...
    def _perform_code_owned(self, r):
        ...
        if role == 'B':
            ...
            if receipt:
                ...
                if before != {k: manifest[k] for k in ('entries', 'omitted')}:
        ...
        check_prompt(prompt)
        try:
            thread = self.client.new_thread(settings, str(scratch), instructions)
        except Exception as error:
