...
class RpcClient:
    ...
    def scoped_thread(self, settings, cwd, instructions):
        ...
        for server in (config.get('mcp_servers') or {}):
            overrides['mcp_servers.' + json.dumps(server) + '.enabled'] = False
        for plugin in (config.get('plugins') or {}):
            overrides['plugins.' + json.dumps(plugin) + '.enabled'] = False
        response = self.call('thread/start', {
