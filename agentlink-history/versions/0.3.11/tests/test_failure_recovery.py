    ...
    def test_T07_T11_T12_repeated_failures_then_valid_next_job(self):
        ...
        for pair, mode in enumerate(spec['fault_order']):
            ...
            pairs.append({'pair':pair, 'fault':mode, 'failed_job':failed.name, 'failed_status':expected,
                ...
                'failed_job_instances':read_json(failed / 'meta.json').get('participants'),
                'new_job_instances':read_json(completed / 'meta.json').get('participants'),
                'failure_server_pid':failure_pid, 'next_server_pid':a.client.process.pid,
