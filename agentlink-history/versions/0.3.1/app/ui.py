...
class MainWindow(W.QMainWindow):
    ...
    def start_discussion(self):
        ...
        self.service.command('start', topic=topic, rounds=self.rounds.value())
    ...
    def select_history(self, item):
        self.service.command('select', job_id=item.data(QtCore.Qt.UserRole))
    ...
    def update_buttons(self):
        ...
        self.start_button.setEnabled(self.connected and not self.storage_warning and not busy and not (self.peer.get('online') and self.peer.get('job_id')))
    ...
    def export_report(self):
        ...
        if path:
            self.save_text(path, self.snapshot.get('report') or report_text(self.snapshot['meta'], self.snapshot.get('turns', [])))
