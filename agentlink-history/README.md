...
收录本机已有的 **0.3.0–0.3.15，共 16 个版本**，重点保留 0.3.4、0.3.9、0.3.10 的源码、构建文件、说明与测试。归档日期：2026-09-11。
...
| [0.3.11](versions/0.3.11/README.md) | 47 | 29 |
| [0.3.12](versions/0.3.12/README.md) | 48 | 30 |
| [0.3.13](versions/0.3.13/README.md) | 49 | 31 |
| [0.3.14](versions/0.3.14/README.md) | 53 | 32 |
| [0.3.15](versions/0.3.15/README.md) | 57 | 34 |
...
本批补齐此前远程缺少的五个版本，保持 Windows 交付 ZIP 中 source/ 的原始字节；源码包含 app/、tests/、packaging/ 和构建说明。每版 evidence/ 中另外保存原交付报告和批次根目录的测试、预检脚本；这些脚本可能引用原机绝对路径，完整运行日志仍在原机。报告中的通过数字是历史执行记录，本次同步未重新运行功能测试。

- 0.3.11：项目源码快照交付与独立审查。
- 0.3.12：项目权限预检与 MCP 配置兼容修正；后续权限配置问题参见 0.3.13。
- 0.3.13：权限配置兼容修正。
- 0.3.14：Windows Codex 运行组件发现及修复工具。
- 0.3.15：节点定向输入、人工恢复未完成步骤并接回原流程。

0.3.15 在本归档中指已交付基线，不包含其他本地工作区尚未提交的后续修复草稿。收录版本不表示已完成所有缺陷修复或真实双机验收。此次同步不包含账号凭据、生产配置、讨论数据库、运行时二进制或完整运行日志；安装包本身仍在原发布位置。
