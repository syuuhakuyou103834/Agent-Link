...
EXTRACT=ROOT/'p2'
archive=ROOT/'dist/AgentLink-GUI-0.3.7-Windows-x64.zip'
...
checks=[]
for name,command in [('identity',[str(EXTRACT/'runtime/python.exe'),'-c','import app;assert app.__version__=="0.3.7";print(app.__version__)']),
...
data=ROOT/'launcher-native/AgentLinkGUI';data.mkdir(parents=True)
(data/'settings.json').write_text(json.dumps({'role':'B','auto_connect':False,'model':'test-preserved',
...
result={'zip':str(archive),'sha256':digest(archive),'verified_files':len(files),'files':files,
    ...
    'supersedes':'package.py WMI child discovery timeout; product code unchanged for native probe'}
(ROOT/'evidence/package-verification.json').write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
(ROOT/'dist/SHA256SUMS.txt').write_text(digest(archive)+'  '+archive.name+'\n',encoding='ascii')
