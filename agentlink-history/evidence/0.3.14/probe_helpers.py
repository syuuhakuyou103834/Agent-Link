...
sys.path.insert(0,str(root/'source') if sys.argv[1]=='after' else r'C:\AgentLink-hotfix-0.3.13\source')
...
c=RpcClient([str(root/'runtime-fixture'/'codex.exe'),'app-server','--listen','stdio://'],root/('rpc-'+sys.argv[1]))
methods=[];orig=c.call
...
(root/'evidence'/('real-'+sys.argv[1]+'.json')).write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding='utf-8')
