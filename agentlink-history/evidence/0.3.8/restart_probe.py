...
def host(folder, mode, recovering):
    ...
    until(lambda:all(read_json(shared/'nodes'/(r+'.json'),{}).get('instance')==n.instance
                    and read_json(shared/'nodes'/(r+'-owner.json'),{}).get('instance')==n.instance
    ...
    try:
        if recovering:
            ...
            assert len(calls(folder)) == before_calls
            until(lambda:any(k=='error' and '旧场次' in v.get('message','') for k,v in events))
