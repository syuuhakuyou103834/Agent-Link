...
class ProjectDialog(W.QDialog):
    def __init__(self, data, settings, parent=None):
        super().__init__(parent)
        self.store, self.settings = Projects(data), settings
        self.setWindowTitle('项目管理 · ' + settings.role)
        self.resize(760, 430)
        layout = W.QVBoxLayout(self)
        form = W.QFormLayout(); layout.addLayout(form)
        self.select = W.QComboBox(); self.select.addItem('创建新项目' if settings.role == 'A' else '选择 A 发布的项目', None)
        local = self.store.all()
        choices = local if settings.role == 'A' else read_json(Path(settings.shared_root) / 'projects' / 'A.json', {}).get('projects', [])
        for p in choices:
            self.select.addItem(p['name'] + ' · ' + p['id'][:8], p['id'])
        form.addRow('项目', self.select)
        self.name = W.QLineEdit(); form.addRow('项目名称', self.name)
        self.directory = W.QLineEdit(); row = W.QHBoxLayout(); row.addWidget(self.directory)
        def choose():
            path = W.QFileDialog.getExistingDirectory(self, '选择项目目录', self.directory.text())
            if path: self.directory.setText(path)
        row.addWidget(button('选择目录', choose))
        form.addRow('A 源码目录' if settings.role == 'A' else 'B 接收快照的根目录', row)
        self.dependencies = W.QPlainTextEdit(); self.dependencies.setMaximumHeight(100)
        self.dependencies.setPlaceholderText('可选：每行一个明确需要的本机依赖目录，仅授予读取权限。')
        form.addRow('本机只读依赖', self.dependencies)
        def selected():
            identifier = self.select.currentData()
            existing = next((p for p in local if p['id'] == identifier), None)
            remote = next((p for p in choices if p['id'] == identifier), None)
            self.name.setText((existing or remote or {}).get('name', ''))
            default = str(Path(data).parent / 'AgentLink-Reviews' / identifier) if identifier and settings.role == 'B' else ''
            self.directory.setText((existing or {}).get('directory', default))
            self.dependencies.setPlainText('\n'.join((existing or {}).get('dependencies', [])))
        self.select.currentIndexChanged.connect(selected)
        label = W.QLabel('A 只在所选目录修改源码；B 只读接收完整快照，临时测试写入独立目录。\n不会开放 A 的整台电脑。凭据和依赖缓存不交付，排除清单会随快照展示。')
    ...
    def accept(self):
        try:
            if self.settings.role == 'B' and not self.select.currentData():
                raise ValueError('请先选择 A 已发布的项目')
            separate(self.directory.text(), self.settings.shared_root)
            self.store.save(self.name.text(), self.directory.text(), self.settings.role,
                            self.select.currentData(), self.dependencies.toPlainText().splitlines())
            if Path(self.settings.shared_root).is_dir():
                self.store.publish(self.settings.shared_root, self.settings.role)
        except (ValueError, OSError) as error:
            W.QMessageBox.warning(self, '项目绑定失败', str(error)); return
        super().accept()
...
class MainWindow(W.QMainWindow):
    ...
    def build(self):
        ...
        main.addLayout(top)
        project_row = W.QHBoxLayout()
        self.mode = W.QComboBox(); self.mode.addItem('文字讨论', 'text'); self.mode.addItem('项目代码审查', 'code')
        project_row.addWidget(self.mode)
        self.project_select = W.QComboBox(); project_row.addWidget(self.project_select, 1)
        self.project_button = button('管理项目', self.manage_projects); project_row.addWidget(self.project_button)
        self.issues_button = button('更新问题状态', self.change_issue_status); project_row.addWidget(self.issues_button)
        ...
        self.project_view = browser(); self.tabs.addTab(self.project_view, '源码与项目问题')
        chat = W.QWidget(); chat_layout = W.QVBoxLayout(chat)
        chat_layout.addWidget(W.QLabel('定向输入仅送给选定节点；双方可见审计记录。恢复会追加模型请求，原失败记录保留。'))
        self.node_chat_view = browser(); chat_layout.addWidget(self.node_chat_view, 1)
        self.node_message = W.QPlainTextEdit(); self.node_message.setMaximumHeight(110)
        self.node_message.setPlaceholderText('运行中：调整该节点的方向。失败后：输入恢复要求，例如“额度已恢复，请核查已有结果后继续未完成步骤”。')
        chat_layout.addWidget(self.node_message)
        chat_row = W.QHBoxLayout(); self.node_target = W.QComboBox(); self.node_target.addItems(['A','B'])
        chat_row.addWidget(W.QLabel('目标节点')); chat_row.addWidget(self.node_target)
        self.node_send = button('发送给该节点', self.send_node_message); chat_row.addWidget(self.node_send)
        self.node_recover = button('恢复该节点并继续流程', self.recover_node); chat_row.addWidget(self.node_recover)
        chat_layout.addLayout(chat_row); self.tabs.addTab(chat, '节点对话与恢复')
    ...
    def refresh_projects(self):
        current = self.project_select.currentData()
        self.project_select.clear(); self.project_select.addItem('选择项目', None)
        local = Projects(self.data).all()
        choices = list(local)
        if self.settings.role == 'B':
            choices += [p for p in getattr(self, 'shared_projects', []) if p['id'] not in {v['id'] for v in local}]
        for p in choices:
            self.project_select.addItem(p['name'] + ' · ' + p['id'][:8], p['id'])
        self.project_select.setCurrentIndex(max(0, self.project_select.findData(current)))

    def manage_projects(self):
        try:
            dialog = ProjectDialog(self.data, self.settings, self)
            if dialog.exec_() == W.QDialog.Accepted:
                self.refresh_projects()
        except (OSError, ValueError) as error:
            W.QMessageBox.warning(self, '无法读取项目', str(error))

    def change_issue_status(self):
        issues = self.snapshot.get('project_issues', [])
        if not issues:
            W.QMessageBox.information(self, '项目问题', '本场项目没有已记录的问题。'); return
        selected, ok = W.QInputDialog.getItem(self, '选择问题', '项目问题', [i['id'][:8] + ' · ' + i['description'] for i in issues], 0, False)
        if not ok: return
        item = next(i for i in issues if selected.startswith(i['id'][:8]))
        status, ok = W.QInputDialog.getItem(self, '问题状态', '仅更新记录，不自动发起修改任务', ['待处理', '已解决', '延后'], 0, False)
        if ok:
            self.service.command('issue_status', project=self.snapshot['meta']['project']['id'], issue_id=item['id'],
                                 status={'待处理':'open', '已解决':'resolved', '延后':'deferred'}[status])

    def mode_changed(self):
        code = self.mode.currentData() == 'code'
        self.rounds.setMaximum(3 if code else 8)
        self.rounds.setValue(3 if code else 1)
        self.project_select.setEnabled(code)
        if code:
            self.service.command('project_catalog')
            self.refresh_projects()
    ...
    def start_discussion(self):
        ...
        if mode == 'code' and not project:
            self.notice.setText('请创建或选择项目后再发起代码审查。'); self.notice.show(); self.update_buttons(); return
    ...
    def send_node_message(self):
        text = self.node_message.toPlainText().strip()
        if not text:return
        self.service.command('node_input', target=self.node_target.currentText(), text=text)
        self.node_message.clear()

    def recover_node(self):
        meta=self.snapshot.get('meta',{});text=self.node_message.toPlainText().strip()
        if not text:
            self.node_message.setFocus();return
        self.service.command('start', topic=meta.get('topic',''), rounds=meta.get('rounds',1), mode='code',
            project=meta.get('project',{}).get('id'),
            recovery={'parent_id':meta.get('id'), 'target':self.node_target.currentText(), 'text':text})
    ...
    def receive(self, kind, value):
        if kind == 'status':
            ...
            if self.selection_pending and value.get('meta', {}).get('id') != self.selection_pending:
                return
            self.selection_pending = None
            incoming = value.get('meta', {})
            wanted = self.mode.findData(incoming.get('mode', 'text'))
            if self.mode.currentIndex() != wanted:
                self.mode.setCurrentIndex(wanted)
            if incoming.get('project'):
                self.refresh_projects()
                self.project_select.setCurrentIndex(max(0, self.project_select.findData(incoming['project']['id'])))
        ...
        elif kind == 'project_scope':
            set_html(self.project_view, '<h3>本轮文件范围</h3>' + text_html(json.dumps(value, ensure_ascii=False, indent=2)))
        elif kind == 'project_catalog':
            self.shared_projects = value
            self.refresh_projects()
        elif kind == 'error':
    ...
    def update_buttons(self):
        ...
        failed = self.snapshot.get('status') == 'failed'
        self.node_send.setEnabled(self.connected and busy and not failed)
        self.node_recover.setEnabled(self.connected and not busy and failed and self.snapshot.get('meta',{}).get('mode')=='code' and not self.selection_pending)
        ...
        elif selected and failed and meta.get('mode')=='code':
            self.context_hint.setText('本场失败记录已保留。额度或环境恢复后，可在“节点对话与恢复”继续未完成步骤，不重跑已完成步骤。')
        elif selected:
        ...
        self.project_select.setEnabled(not busy and not selected and self.mode.currentData() == 'code')
        self.mode.setEnabled(not busy and not selected)
        self.issues_button.setEnabled(not busy and self.connected and bool(meta.get('project')))
    ...
    def render(self):
        ...
        for role, pane in self.panes.items():
            ...
            for turn in turns:
                ...
                if turn.get('review'):
                    review = turn['review']
                    readable = [{'passed': '审查通过', 'changes_requested': '需要修改', 'blocked': '审查阻塞'}[review['decision']], review['summary']]
                    for key, label in (('scope', '实际审查文件'), ('tests', '本机测试证据'), ('unverified', '未验证事项'), ('unrelated_issues', '后续待处理问题')):
                        readable += ['', label + '：', '\n'.join(review[key]) or '无']
                    readable += ['', '快照 SHA-256：' + review['snapshot']]
                    body = [{'text': '\n'.join(readable)}]
                title = {'implement': 'A 实施并交付源码', 'review': 'B 独立审查', 'summary': 'A 只读总结'}.get(turn.get('phase'), turn_title(turn.get('index', 0))) + ' · ' + STATUS.get(turn.get('status'), '生成中')
        ...
        if meta.get('mode') == 'code':
            outcome = self.snapshot.get('state', {}).get('outcome')
            if outcome:
                self.state_label.setText(self.state_label.text() + ' · ' + {'passed': '审查通过', 'blocked': '审查阻塞', 'needs_user_decision': '待用户裁决'}.get(outcome, outcome))
            details = {'项目': meta['project']['name'], '项目编号': meta['project']['id'],
                '源码交付': [{'轮次': t.get('revision'), '清单哈希': t.get('artifact', {}).get('manifest_sha256'),
                    '内容': t.get('delivery')} for t in complete if t.get('phase') == 'implement'],
                '项目问题': self.snapshot.get('project_issues', []),
                '本机文件范围': self.snapshot.get('local_scope')}
            lines = ['项目：' + meta['project']['name'], '项目编号：' + meta['project']['id']]
            scope = self.snapshot.get('local_scope') or {}
            if scope:
                lines += ['', '本机源码目录：' + scope['source'], '本机临时测试目录：' + scope['scratch']]
                lines += ['当前文件权限：']
                lines += [path + ' · ' + {'read': '只读', 'write': '可读写', 'deny': '不可访问'}.get(access, access)
                          for path, access in scope.get('permissions', {}).get('filesystem', {}).items()]
            for t in complete:
                if t.get('phase') != 'implement': continue
                delivery = t.get('delivery', {})
                lines += ['', '第 ' + str(t.get('revision')) + ' 轮源码交付',
                          '文件数：' + str(delivery.get('files', 0)) + '；变更条目：' + str(delivery.get('changed_count', 0)),
                          '清单 SHA-256：' + t['artifact']['manifest_sha256'],
                          '完整清单：' + str(self.data / 'runs' / meta['id'] / (delivery.get('complete_manifest') or 'manifest.json')),
                          '变更路径（最多显示 200 项）：', *delivery.get('changed', []),
                          '排除条目：' + str(delivery.get('omitted_count', 0))]
                lines += [v['path'] + ' · ' + v['reason'] for v in delivery.get('omitted', [])]
            lines += ['', '项目后续问题：']
            if self.snapshot.get('project_issues_source') == 'cache':
                lines += ['离线缓存，可能不是最新状态。']
            elif self.snapshot.get('project_issues_source') == 'unavailable':
                lines += ['本机尚未缓存问题清单；连接后可读取。']
            lines += [{'open': '待处理', 'resolved': '已解决', 'deferred': '延后'}.get(i['status'], i['status']) + ' · ' + i['description']
                      for i in self.snapshot.get('project_issues', [])] or ([] if self.snapshot.get('project_issues_source') == 'unavailable' else ['无'])
            set_html(self.project_view, text_html('\n'.join(lines)))
        labels={'queued':'等待该节点接收','send_pending':'正在发送，尚未确认','delivered':'已送达运行中请求',
                'rejected':'接口拒绝','uncertain':'送达不确定，未自动重发','included_next_request':'已加入下一请求，发出情况见调用台账'}
        lines=['累计请求尝试（含失败）：'+str(self.snapshot.get('request_attempts',0)), ''] if self.snapshot.get('meta',{}).get('mode')=='code' else []
        recovery=self.snapshot.get('meta',{}).get('recovery')
        if recovery:
            lines += ['恢复来源：'+recovery['parent_id'], '已承接完成步骤：'+str(recovery['index']),
                      '累计请求尝试（含失败）：'+str(self.snapshot.get('request_attempts',0)), '']
        for message in self.snapshot.get('node_inputs',[]):
            lines += ['给节点 '+message.get('target','?')+' · '+labels.get(message.get('state'),message.get('state','')),
                      message.get('text',''),message.get('detail',''),'']
        set_html(self.node_chat_view,text_html('\n'.join(lines) or '暂无定向消息。失败后请选择未完成步骤所属节点，再输入恢复要求。'))
        self.render_execution()
        self.update_buttons()
    ...
    def copy_session(self):
        sessions = sorted({t.get('thread_id') for t in (self.snapshot.get('turns', []) + self.snapshot.get('live', [])) if t['role'] == self.settings.role and t.get('thread_id')})
        if not sessions:
