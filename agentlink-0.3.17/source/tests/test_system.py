...
class SystemTests(unittest.TestCase):
    ...
    def test_B_cannot_initiate(self):
        self.assertFalse(self.nodes['B'].command('start',topic='B must not initiate',rounds=1))
        time.sleep(.2)
        self.assertIsNone(self.job())
        self.assertFalse(self.calls('B'))
    ...
    def test_continuation_both_nodes_restart_reverse_and_fresh(self):
        ...
        fresh = self.start_and_finish('A', 'FRESH-UNRELATED: 独立新任务')
