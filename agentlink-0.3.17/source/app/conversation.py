...
from .context import check_prompt, load_context
...
class UnifiedWorkflow:
    ...
    def _conversation_context(self, value):
        # Full persisted history; exceeding the input bound is explicit, never silent truncation.
        text = json.dumps({'messages': value['messages'], 'confirmed': value['confirmed'],
                           'completed_rounds': value['completed_rounds'], 'phase': value['phase']}, ensure_ascii=False)
        check_prompt(text)
        return text
    ...
    def _unified_compatible(self):
        ...
        if FEATURE not in peer.get('features', []):
            raise ValueError('统一对话需要双方升级至 0.3.17；旧版本不能执行新任务')
    ...
    def _unified_chat(self, value, msg):
        ...
        prompt = ('[AGENTLINK_CONVERSATION]\n' + self._conversation_context(value) +
    ...
    def _consume_unified_input(self, value):
        ...
        for key in value['pending']:
            ...
            if value['phase'] == 'chat_interrupted' and word not in RESUME_WORDS:
                continue
    ...
    def _start_unified_job(self, value):
        ...
        with FileLock(self.box.root / 'discussion.lease'):
            ...
            self.box.put(meta['id'], 'conversation-context.json', {'text':self._conversation_context(value)})
            if plan:
                self.active = meta
                try:
    ...
    def _unified_text_step(self, request):
        ...
        try:
            ...
            prompt = '[AGENTLINK_FORMAL_TEXT]\n阶段：' + phase + self.unified_formal_context()
            prompt += '\n之前结果：' + json.dumps(request['previous'], ensure_ascii=False)
            if phase == 'review':
            ...
            ledger = dict(job_id=job, role=role, step=step, phase=phase, thread_id=thread,
                          host=socket.gethostname(), time=now(), status='send_pending', prompt_sha256=fingerprint(prompt))
